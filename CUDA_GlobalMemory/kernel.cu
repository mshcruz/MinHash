...
{
  const unsigned int tid = threadIdx.x + blockDim.x * blockIdx.x;
  extern __shared__ int s_wordsHashes[];
  //  int temp = lwSize/THREADS_PER_BLOCK;
  //  if (numSets % THREADS_PER_BLOCK) numberOfBlocks++;

  if (tid < numSets) { //OOB Control
    int offSetSM, offSetCM, offSetHM, minhashValue;
    for (int i = 0; i < numHashFunctions; i++) {
      //Bring hashes to shared memory
      for (int j = 0; j < lwSize; j=j+THREADS_PER_BLOCK) {
	if ((threadIdx.x+j) < lwSize) { //OOB Control
	  s_wordsHashes[threadIdx.x+j] = d_hashMatrix[threadIdx.x+j+(i*lwSize)]; //Coalesced
	}
      }
      __syncthreads();
      offSetSM = tid + (i*numSets); 
      minhashValue = d_signatureMatrix[offSetSM]; //Coalesced
      offSetCM = d_cmColPtr[tid]; //Coalesced
      //Find out what words belong to the set
      for (int j = offSetCM; j < d_cmColPtr[tid+1]; j++) {
	offSetHM = d_cmRowIdx[j]; //Not coalesced
	//Calculate the smallest hash value (minhash)
	minhashValue = min(s_wordsHashes[offSetHM],minhashValue);
	  //	if (minhashValue > s_wordsHashes[offSetHM]) { //Not coalesced
	  //	  minhashValue = s_wordsHashes[offSetHM];
	  //	}
	//Update the signature matrix with the minhash
//	  printf("New value: %d - Old value: %d\n", d_hashMatrix[offSetHM], d_signatureMatrix[offSetSM]);
	...
	*/
      ...
      d_signatureMatrix[offSetSM] = minhashValue; //Coalesced
      __syncthreads();
    }
  }
}
...
nestedLoopJoin_kernel(int* d_signatureMatrix, int rSize, int sSize, int numHashFunctions, float threshold, int *d_similarPairsCount)
{
  ...
  if (tid < rSize) { //OOB Control
    ...
    for (int i = 0; i < numHashFunctions; i++) {
      offSetSM = tid + (i*numSets); 
      smFixedRow[i] = d_signatureMatrix[offSetSM]; //Serial/Coalesced
    }
	...
	offSetSM = j + (k*numSets); 
        smVariableRow[k] = d_signatureMatrix[offSetSM]; //Serial/Not Coalesced
      }
...
kernelManager(vector<int> &h_signatureMatrix, ccsMatrix* h_characteristicMatrix, int lwSize, int numHashFunctions, int sSize, int rSize, vector<string> relationRSetsID, vector<string> relationSSetsID)
{
  ...
  */
  ...
  updateSignatureMatrix_kernel<<<numberOfBlocks, numberOfThreads, sizeof(int)*lwSize>>>(d_signatureMatrix, d_hashMatrix, d_cmRowIdx, d_cmColPtr, lwSize, smSize, numSets, numHashFunctions);
